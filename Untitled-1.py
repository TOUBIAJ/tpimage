...
# Normaliser l'histogramme dans l'intervalle [0, 1]
histogram_normalized = histogram / float(image.size)

# Afficher l'histogramme d'origine
...
# Calcul des histogrammes des images originale et égalisée
...
# Définir un filtre générique 3x3
filtre = np.array([[0, 1/7, 0],
                   [1/9, 1/9, 1/9],
                   [0, 1/7, 0]])
...
def filtrage(image, filtre):
    ...
    # Appliquer le filtre en excluant les bords
    for i in range(1, hauteur - 1):
        for j in range(1, largeur - 1):
            fenetre = image[i-1:i+2, j-1:j+2]
            somme_ponderee = np.sum(fenetre * filtre)
...
# Afficher les images originale et filtrée avec des titres
font_color = (0, 0, 255)
...
# Afficher les histogrammes avec cv2_imshow
